...
export const SessionView = ({
  appConfig,       // kept in signature so it doesn't land on the DOM via ...props
  onAnimationComplete, // same
  ...props
  ...
  // Section ref typed as HTMLElement so it works with both <section> and VariableProximity
  ...
  const phaseText  = { listening: 'LISTENING', reasoning: 'REASONING', speaking: 'SPEAKING' }[phase] ?? 'IDLE';
  const phaseColor = { listening: 'text-emerald-400', reasoning: 'text-cyan-400', speaking: 'text-blue-400' }[phase] ?? 'text-white/40';

  return (
    <section
      ref={containerRef as React.RefObject<HTMLElement> & React.RefObject<HTMLDivElement>}
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
        <motion.div
          animate={{
            scale:   phase === 'speaking' ? 1.5 : phase === 'listening' ? 1.3 : 1,
            opacity: phase === 'speaking' ? 0.35 : phase === 'listening' ? 0.25 : 0.1,
          }}
          transition={{ duration: 1.5, ease: 'easeInOut' }}
          className="absolute top-1/2 left-1/2 h-[700px] w-[700px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-cyan-500/25 blur-[140px]"
        />
        <div className="absolute -right-[10%] -top-[10%] h-[350px] w-[350px] rounded-full bg-blue-600/10 blur-[90px]" />
        <div className="absolute -bottom-[10%] -left-[10%] h-[350px] w-[350px] rounded-full bg-indigo-600/10 blur-[90px]" />
      </div>
      ...
      <motion.div
        ...
        className="absolute inset-x-0 top-20 z-50 flex items-center justify-between px-6 md:top-24 md:px-10"
      >
        <div className="flex items-center gap-5">
          ...
          <div className="hidden md:block">
            <VariableProximity
              ...
              className={`text-xs font-black tracking-[0.4em] transition-colors duration-500 ${phaseColor}`}
            />
          </div>
        </div>
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center overflow-hidden pt-32 pb-32">
        ...
        <div className="pointer-events-none absolute right-4 top-1/2 z-40 hidden -translate-y-1/2 flex-col gap-4 lg:flex md:right-8">
          <SpotlightCard className="pointer-events-auto w-56 border-white/8 bg-black/40 p-4 backdrop-blur-xl">
            <span className="mb-3 block text-[9px] font-black tracking-widest text-white/25 uppercase">
              System Metrics
            </span>
      ...
      <motion.div
        ...
        className="absolute bottom-8 left-1/2 z-50 flex w-full max-w-fit -translate-x-1/2 justify-center px-4"
      >
        <SpotlightCard className="overflow-visible rounded-2xl border-white/10 bg-black/50 backdrop-blur-2xl">
